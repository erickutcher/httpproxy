...
SECURITY_STATUS SSL_WSAAccept( SOCKET_CONTEXT *context, bool &sent )
{
	...
	if ( context != NULL && context->ssl != NULL )
	{
		...
		int nRet = _WSARecv( ssl->s, &encrypted_buf, 1, NULL, &dwFlags, &context->overlapped_read, NULL );
		...
		if ( scRet == SEC_I_CONTINUE_NEEDED || scRet == SEC_E_INCOMPLETE_MESSAGE || scRet == SEC_I_INCOMPLETE_CREDENTIALS ) 
		{
			...
			if ( scRet == SEC_E_OK || scRet == SEC_I_CONTINUE_NEEDED || FAILED( scRet ) && ( dwSSPIOutFlags & ISC_RET_EXTENDED_ERROR ) )
			{
				if ( ssl->acd.OutBuffers[ 0 ].cbBuffer != 0 && ssl->acd.OutBuffers[ 0 ].pvBuffer != NULL )
				{
					...
					context->wsabuf_write.buf = ( char * )ssl->acd.OutBuffers[ 0 ].pvBuffer;
					context->wsabuf_write.len = ssl->acd.OutBuffers[ 0 ].cbBuffer;

					context->current_operation_write = IO_Write;

					int nRet = _WSASend( ssl->s, &context->wsabuf_write, 1, NULL, 0, &context->overlapped_write, NULL );
					if ( nRet == SOCKET_ERROR && ( _WSAGetLastError() != ERROR_IO_PENDING ) )
					{
						sent = false;
						g_pSSPI->FreeContextBuffer( ssl->acd.OutBuffers[ 0 ].pvBuffer );
...
SECURITY_STATUS SSL_WSAShutdown( SOCKET_CONTEXT *context, bool &sent )
{
	...
	if ( context != NULL && context->ssl != NULL )
	{
		...
		scRet = g_pSSPI->AcceptSecurityContext(
						( context->context_type == CONTEXT_TYPE_SERVER ? &g_hCreds_server : &g_hCreds_client ),
						&ssl->hContext,
						NULL,
						dwSSPIFlags,
						SECURITY_NATIVE_DREP,
		...
		if ( ssl->sdd.OutBuffers[ 0 ].pvBuffer != NULL && ssl->sdd.OutBuffers[ 0 ].cbBuffer != 0 )
		{
			...
			context->wsabuf_write.buf = ( char * )ssl->sdd.OutBuffers[ 0 ].pvBuffer;
			context->wsabuf_write.len = ssl->sdd.OutBuffers[ 0 ].cbBuffer;
...
SECURITY_STATUS SSL_WSASend( SOCKET_CONTEXT *context, WSABUF *send_buf, bool &sent )
{
	...
	if ( context != NULL && context->ssl != NULL )
	{
		...
		context->wsabuf_write.buf = ( char * )ssl->sd.pbDataBuffer;
		context->wsabuf_write.len = Buffers[ 0 ].cbBuffer + Buffers[ 1 ].cbBuffer + Buffers[ 2 ].cbBuffer; // Calculate encrypted packet size
...
SECURITY_STATUS SSL_WSARecv( SOCKET_CONTEXT *context, bool &sent )
{
	...
	if ( context != NULL && context->ssl != NULL )
	{
		...
		if ( ssl->cbIoBuffer == 0 || ssl->rd.scRet == SEC_E_INCOMPLETE_MESSAGE )
		{
			...
			nRet = _WSARecv( ssl->s, &encrypted_buf, 1, NULL, &dwFlags, &context->overlapped_read, NULL );
			if ( nRet == SOCKET_ERROR && ( _WSAGetLastError() != ERROR_IO_PENDING ) )
			{
				sent = false;
